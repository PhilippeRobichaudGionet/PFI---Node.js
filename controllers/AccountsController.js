                ...
                if (user != null) {
                    if (user.Password == loginInfo.Password) {
                        if (user.Authorizations.readAccess == -1 && user.Authorizations.writeAccess == -1){
                            this.HttpContext.response.notAloud("Compte bloqué par l’administrateur");
                        }
                        else{
                    ...
                    this.HttpContext.response.userNotFound("Courriel d'utilisateur introuvable");
            } else
                this.HttpContext.response.notImplemented();
        } else
            this.HttpContext.response.badRequest("Courriel et mot de passe manquant");
    }
    logout() {
